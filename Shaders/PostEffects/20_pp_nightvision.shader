...
const vec2 lensRadius 	= vec2(0.65, 0.0);              //Radius and feathering of vignette                     //
...
void main() 
{
	...
	//Noise
        float noise =  (fract(sin(dot(tcoord ,vec2(12.0,78.0)+currenttime)) * 43758.0));
	...
	//Render
	fragData0 = (vec4(gray * nvcol, texcolor.a) + noise*noiseamount) * vec4(vigfin,vigfin,vigfin, 1.0);
}
