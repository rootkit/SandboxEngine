...
void main(void)
{
	...
	fragData0 = texture(texture9,ex_texcoords0);
	if (fragData0.a<0.5) discard;
	fragData0 *= ex_color;
	//fragData0 = vec4(1.0,0.0,0.0,1.0);
	
	//Normal map
	vec3 normal = ex_normal;
	normal = texture(texture10,ex_texcoords0).xyz * 2.0 - 1.0;
	normal.xz *= -1.0;
	...
	vec4 emission = texture(texture11,ex_texcoords0);
	
	//On Intel graphics this comes out red for some reason
	fragData2 = vec4(0.0,0.0,0.0,emission.a);
}
