...
void main()
{
	...
	float dist=distance(vec4(playerPos,modelvertexposition.w),modelvertexposition);
	if (dist<playerRadius) {
		float height=max(modelvertexposition.y-playerPos.y,0);
		float theta=atan(modelvertexposition.x-playerPos.x,modelvertexposition.z-playerPos.z);
		float delta=1.57*(playerRadius-dist)/playerRadius;
		gl_Position = projectioncameramatrix * (modelvertexposition-vec4(0,modelvertexposition.y,0,0)+height*vec4(sin(theta)*sin(delta),cos(delta),cos(theta)*sin(delta),0));
	}
	else {
