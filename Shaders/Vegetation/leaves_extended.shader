...
uniform sampler2D texture7;// season palette
...
void main()
{
	...
	vec4 seasonPosition = modelvertexposition;			
	...
	float colorSeed=entitymatrix_[0][0]*1000;
	float colorSample=((int(playerSeason*100.0)+(int(colorSeed))%12)%100)/100.0;
	ex_color=texture(texture7,vec2(colorSample,0));
...
void main(void)
{
	...
	//Season code
	vec4 newcolor=(.3+materialcolordiffuse)*(.7+ex_color);

	outcolor *= newcolor;
	
	if (ex_color.a<.5) discard;
